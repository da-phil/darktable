...
Two properties of that scheme cause the artifacts reported on detailed,
...
2. **The saturation envelope is computed from a 3×3 spatial block**, so a
   pixel's weight depends on its neighbors. In high-frequency texture the
...
> artifacts, the **CFA false color** on changing content (waves, foliage —
> [§3.2](#32-luminance-guided-weighting-avoids-cfa-false-color)) and the
...
> best-exposed frame (no fragmented ghosts, no motion false color), at the cost
...
### 3.2 Luminance-guided weighting (avoids CFA false color)
...
**mosaiced CFA**: each pixel is a single color (R, G *or* B). If the weight
came from a pixel's own value, then where content moves between frames (waves,
foliage) neighboring R/G/B could be sourced from *different* frames — and,
...
vivid **magenta / green / purple** false color.
...
(all four pixels of a Bayer quad get the same `L`). Neighboring R/G/B are then
merged with *identical* per-frame weights, so they always come from the same
frame mixture and cannot break into false color. The value being merged is
...
A worked moving-water example (neutral gray, 3-stop bracket, a wave crest
...
§3.2 keeps neighboring channels *consistent*, but it does not by itself fix the
...
recoverable highlights keep their detail and color. Worked example (3-stop
...
color matrix still come from the first collected frame; for a real bracket
...
| `deghost_threshold` | float | `0.0` | `0` = off. `>0` *feathers out*, per pixel in pass 2, any frame whose **luminance** deviates from the **reference frame**, reaching full rejection at this fraction (e.g. `0.4`), so moving content resolves to the reference's view (coherent, no fragments/false color). **Off by default — set a positive value to enable de-ghosting at all.** |
...
Luminance-guided weighting ([§3.2](#32-luminance-guided-weighting-avoids-cfa-false-color))
...
[§3.2](#32-luminance-guided-weighting-avoids-cfa-false-color)).
...
that makes the result **spatially incoherent** — neighboring pixels resolve to
...
(no fragments) and **self-consistent across CFA channels** (no false color).
...
convex blend of the frames, so it cannot manufacture false color on its own —
but the *selection* is decided independently at each quad. Where it jumps between
neighboring quads (moving content, noise near a threshold) it stamps a quad-grid
...
**Staying in the CFA/RAW domain.** A Gaussian blur must never mix color
...
demosaicer inventing color. Re-interleaving the four collapsed planes yields the
...
**Log-domain blend (no color casts).** A multi-scale blend spreads any
...
the brighter the disagreement, the larger the reduction (a 5× mover's color
...
the partition of unity — hence the no-color-cast guarantee — is preserved. The
...
radiance, which desaturates the highlight to gray — and gray, after white
...
room. The whole highlight then resolves to that one frame, so it keeps its color
and its smooth rolloff — i.e. it matches developing the darkest bracket directly,
rather than inventing a gray/magenta blend.

**A clipping frame defers its color to the reference, and its brightness where
...
**coarse** levels still leak that capped color into the neighborhood (the
...
head-room — **splitting color from brightness**:

- **Color** — always adopt the reference's channel ratios. This repairs the capped
  ...
  **coronae** of clipped discs, where own and reference luminance are close (so a
  purely brightness-gated substitution would leave the color, and its magenta,
...
that still-colored corona (it converges to the shortest exposure, above), so the
...
| CFA false color on motion | avoided by averaging everything (blurry) | avoided by luminance-guided weights (sharper, color-correct) |
| Magenta blown highlights | fully-clipped pixels forced to white | unrecoverable pixels forced to white — `linear`/GPU path: brightest channel clips every frame; `pyramid` default: the shortest exposure's *weakest* channel, keeping the colored corona ([§3.8](#38-multi-scale-laplacian-pyramid-blend)) |
...
| `data/kernels/programs.conf` | registered `hdrmerge.cl` as program `44` |
| `data/darktableconfig.xml.in` | registered the four `plugins/lighttable/hdrmerge/*` config keys |
...
- **Luminance-guided weighting** — fixes CFA false color on moving content
  ([§3.2](#32-luminance-guided-weighting-avoids-cfa-false-color)); always on.
...
- **De-ghosting** — opt-in *reference-frame* soft fall-off: moving content
  ...
  / false color), median fallback where the reference is clipped/black, config
...
- **Spatial / multi-scale blending (the `enfuse` model).** This whole algorithm
  ...
  1. it works on the **already-demosaiced, color-managed, gamma-encoded LDR**
...
- **Saturated single-channel highlights (linear / GPU path).** On the
  ...
  channel, so a *saturated color* whose dominant channel clips in every frame
...
2. **False-color & magenta fixes** — using the real `hdrmerge.h`: (a) a moving
   neutral-gray bracket — per-channel weighting gives `R/G = 0.51` (cyan cast),
...
4c. **De-ghost consensus** — a synthetic Bayer quad over 5 frames with a green
   ...
   **reference frame** resolves the pixel to one frame coherently: anchored to a
   ...
   false color either way. Static R/B sites stay exactly neutral throughout.
...
4g. **Color-cast (halo) diagnosis + log-domain fix** — a *colored* static scene
...
4h. **Feather + neutralization border** — the `feather` weight-blur keeps the
   colored-static reconstruction exact (0.07 %) and per-channel bias `≈1.0` at
...
4i. **Blown-sun highlights (magenta + hard disc)** — a synthetic colored radial
   ...
   (R/G, B/G ratios) at the default `feather = 0.5` — versus a gray, ~2× larger
...
4j. **Clipped-highlight leak** — a bright colored patch clipped in the long/mid
   ...
   — restores the patch's color and brightness, with the sun corona and all other
   tests unchanged.
4k. **In-tree unit tests** — `src/tests/unittests/test_hdrmerge.c` (cmocka) drives
   ...
   the `pyramid` and `linear` paths reconstruct a frame-identical colored scene to
   `out = E/cal_max` within **1 %** (energy preservation); an all-channel-blown core
   neutralizes to `1.0`; and a bright colored patch clipped in the long/mid
   exposures keeps its color (chroma deviation from the shortest-exposure reference
...
- [x] Luminance-guided weighting (always on) — fixes CFA false color on motion
...
- [x] Blown-highlight fix — highlights converge to the shortest exposure (no
      gray/magenta), neutralization keyed to the weakest channel with a
...
- [x] Clipped highlights fade to the shortest exposure where it is brighter —
      recovers the correct color *and* brightness and stops the coarse-level leak
...
- [x] In-tree cmocka tests `src/tests/unittests/test_hdrmerge.c` — energy
      preservation, highlight neutralization, clipped-channel color/brightness
