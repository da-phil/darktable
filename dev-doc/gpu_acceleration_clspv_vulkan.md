...
**Per-module ports**: 64 modules currently expose `process_vk`,
in three categories.
...
| `src/iop/colorequal.c` | **Partial port — `ce_sample_input` kernel only**. Entry kernel of colorequal's pipeline: reads `dev_in`, computes per-pixel saturation (delta/dmax), luminance via `Y_to_dt_UCS_L_star`, and UV chromaticity via `dt_UCS_xyY → UV` after the input-matrix transform to XYZ_D65. Image-shortcut port: OpenCL's `samplerA` integer-coord reads on `dev_in` translate to flat float4 storage buffer reads. 5 storage bindings (in, saturation, lum, uv, pix_out, mat) + 8 B PC. The matrix is a 12-float flat buffer (3 rows × 4 floats — `.w` padding kept to match the OpenCL `cl_mem_t` 16-byte stride). This port lands the **colour-science helper cohort** needed for the full colorequal port: `vk_matrix_dot`, `vk_dt_D65_XYZ_to_xyY`, `vk_dt_xyY_to_XYZ`, `vk_dt_UCS_LUV_to_JCH`, `vk_dt_UCS_HSB_to_JCH`, `vk_dt_UCS_L_star_to_Y`, `vk_dt_UCS_JCH_to_xyY`, `vk_dt_UCS_HSB_to_XYZ`, `vk_gamut_map_HSB` — all mirroring `colorspace.h` byte-for-byte. The remaining 15 kernels (process_data, write_output, write_visual, draw_weight, the 12-kernel guiding path) need additional storage-buffer allocations and host orchestration; that's the next step but not landed in this commit. `process_vk` isn't wired yet — the kernel is a standalone artefact validated against an independent C reference: saturation **bit-equal (max 0.0)**, luminance at 1.2e-7, uv at 7.5e-8, alpha pass-through bit-exact. |
