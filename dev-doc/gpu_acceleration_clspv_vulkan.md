...
| `src/iop/colorequal.c` | **Non-guiding fast path** (the `use_filter == off` mode — direct per-pixel hue/saturation/brightness equalisation without the guided-filter smoothing). Three kernels chained in `process_vk`: `ce_sample_input` (input → XYZ_D65 → xyY → dt-UCS L*/UV + per-pixel saturation), then `dt_gaussian_mean_blur_vk` smooths the saturation map (1-channel), then `ce_process_data` (LUV → JCH → HSB, looks up the per-hue correction LUTs), then `ce_write_output` (apply corrections, gamut-map, HSB → XYZ_D65 → output RGB via the output matrix). 6/9/6 storage bindings respectively; the LUTs (saturation/hue/brightness/gamut, each `LUT_ELEM`=512 floats) and the two 12-float matrices upload per-dispatch via the batched-upload path. Added the **colour-science helper cohort** to `dt_vulkan_common.h` — `vk_matrix_dot`, `vk_dt_D65_XYZ_to_xyY`, `vk_dt_xyY_to_XYZ`, `vk_dt_UCS_LUV_to_JCH`, `vk_dt_UCS_HSB_to_JCH`, `vk_dt_UCS_L_star_to_Y`, `vk_dt_UCS_JCH_to_xyY`, `vk_dt_UCS_HSB_to_XYZ`, `vk_gamut_map_HSB` — all mirroring `colorspace.h` byte-for-byte. The **guided-filter path** (`use_filter == on`, the default — a 13-kernel chain with covariance / prefilter / correlation passes + bilinear up/downsamples between resolution levels) and the GUI mask-display preview stay on OpenCL / CPU, gated via `commit_params`'s `process_vk_ready = FALSE` (the §10.2 predictive pattern). Validated end-to-end on lavapipe against an independent C reference of the full three-kernel chain (matrices + LUTs + all 9 colour-science conversions): max error 1.8e-6 (single-bit FP) across the whole image, all 4 channels. |
...
- **HARD** — denoiseprofile,
  ...
  `colorequal`'s **non-guiding fast path has landed** (§4.2 — the
  3-kernel `ce_sample_input` → blur → `ce_process_data` →
  `ce_write_output` chain with the colour-science helper cohort);
  the guided-filter path (13 kernels + bilinear up/downsamples) is
  gated to OpenCL / CPU and remains the larger piece. `retouch` is
  now **helper-unblocked** by the
  ...
  iteration).
  `colorequal`'s guided path still needs the ~13-kernel
  orchestration and the bilinear up/downsamples. Done
