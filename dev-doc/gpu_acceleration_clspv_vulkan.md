...
**Per-module ports**: 43 modules currently expose `process_vk`,
in three categories.
...
- **MODERATE** — multi-pass with intermediate buffers or
  ...
  `highpass`, `highlights`. The Gaussian VK
  helper (§5.10) handles the separable-blur half; the larger blurs
  still want workgroup-local-memory plumbing for the cache-friendly
  kernels (note: `highpass` does its own explicit Gaussian
  convolution in OpenCL rather than the Deriche IIR `dt_gaussian_*`,
  so a faithful port can't just reuse `dt_gaussian_blur_vk` — it
  would drift like the removed diffuse approximation; `soften`
  and `sharpen` are the same shape and are now done, see below).
...
4. ✅ **Module ports** (landed; see the §4.2 tables for the full list).
   ...
   a C reference of process_cl), and the Lab-L-channel unsharp
   ...
   blend; bit-equal to the C reference). All are
