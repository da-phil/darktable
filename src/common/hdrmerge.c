...
{
  // negated form so a NaN s (which compares false against everything) is
  // rejected here rather than propagating into the envelope
  if(!(s > 0.0f && s < 1.0f))
    return 0.0f;
...
{
  // negated form so a NaN s is rejected: NaN compares false against both bounds,
  // so the `s <= 0 || s >= 1` spelling would fall through to `(int)f` below,
  // which is undefined for NaN (INT_MIN on x86) and indexes the table far out
  // of bounds
  if(!(s > 0.0f && s < 1.0f))
    return 0.0f;
...
{
  // negated form so a NaN L is rejected (see _hdrmerge_weight_lut)
  if(!(L > 0.0f && L < white))
    return 0.0f;
...
// frame's view -> no fragmented ghosts and no per-channel false color), unlike
...
// jumping between neighboring quads on moving content; that injects a quad-grid
...
// radiance.
//
// CFA correctness: a Gaussian blur must never mix color channels, so each
...
// inventing color. Re-interleaving the four collapsed planes yields the merged
                                        ...
                                        const float inv_white, const float *const wlut)
{
  ...
  // into the colored corona and desaturates it (-> magenta), so feather must not
  // drive it.
  ...
  // clipped under-estimates (which desaturates highlights to gray -> magenta
  // after white balance), so highlights keep the shortest exposure's color and
  ...
  // the recoverable, still-colored corona around a blown highlight instead of
  ...
  // This and the three loops below walk the quad grid in 2D rather than over a
  // flat index: recovering qx/qy from the latter costs a 64-bit hardware divide
  // per quad, and these run n*4 times over the whole half-resolution plane.
  DT_OMP_FOR(collapse(2))
  for(int qy = 0; qy < hh; qy++)
    for(int qx = 0; qx < hw; qx++)
    {
      const size_t q = (size_t)qy * hw + qx;
  ...
  for(int i = 0; i < n; i++)
  {
    ...
    for(int qy = 0; qy < hh; qy++)
      for(int qx = 0; qx < hw; qx++)
      {
        ...
        // reference) so the merge keeps its color/rolloff rather than averaging
        // clipped under-estimates to gray; elsewhere (e.g. deep shadow) keep uniform.
    ...
    // the colored corona, desaturating it into magenta. The handoff is feathered
    ...
    // unity (hence no color bias) holds.
    if(wsigma > 0.0f)
    {
    ...
    // other channels keep their true (lower) values - a color the demosaicer
    ...
    // COARSE levels still leak this capped color into the surrounding highlight.
    ...
    // COLOR from BRIGHTNESS: adopt the reference's color ratios fully (fixes the
    ...
    // half from another (that is itself false color). The fade is a smoothstep on
        ...
        for(int qx = 0; qx < hw; qx++)
        {
          ...
          if(i != ishort && Ls > defer_lo)
          {
            ...
            if(ref_l > eps)
            {
              ...
              // COLOR: always adopt the reference's channel ratios. This repairs the
              ...
              // close, so a brightness-gated substitution would leave the color (and
              ...
              // speckle along a high-contrast edge. g=1: target = ms (color +
              // brightness); g=0: target = ms/ratio (reference color at own luma).
  ...
  // channel clipping), which would whiten the still-recoverable, still-colored
...
void dt_hdrmerge_process_cpu(dt_hdrmerge_t *const h)
{
  ...
  // this is a public entry point (the OpenCL dispatcher's fallback, and what the
  // unit tests drive directly), so it cannot rely on dt_hdrmerge_process() having
  // sanitized the threshold: a non-positive one would make inv_white infinite and
  // turn a black sample into a NaN weight argument.
  ...
  for(size_t p = 0; p < npx; p++)
  {
    ...
    // color channel at this position gets the SAME per-frame weight. This is
    // what keeps neighboring R/G/B from being sourced from different frames
    ...
    // no false color). Frames that disagree with the reference are faded out in
    ...
    // hence identical across color channels.
    num = 0.0f;
    den = 0.0f;
    for(int i = 0; i < n; i++)
    {
...
{
  // hdrmerge.cl is program 44 in data/kernels/programs.conf. The kernel id is
  ...
  const int program = 44; // hdrmerge.cl, from programs.conf
