...
// selects the best-exposed frame more decisively (fewer blend artifacts),
...
typedef struct dt_hdrmerge_t
{
  ...
  // per-frame brightness proxy (fp16): a signal shared by every CFA color
  ...
  // from each channel's own value, so neighboring R/G/B are merged with
  // identical weights and cannot break up into false color where content moves
  ...
  // false-colors on motion; no highlight neutralization).
  ...
  // neutral white core into the colored corona (magenta). Only the pyramid path.
  float feather;
...
static inline dt_hdrmerge_half_t dt_hdrmerge_float_to_half(const float f)
{
  ...
  if(new_exp <= 0) // half denormal or underflow
  {
    ...
    // full_mant is 24 bits, so a total shift of 24 or more discards every bit:
    // bail out at shift > 10 rather than letting `13 + shift` reach 37, which
    // would be an out-of-range shift count on a 32-bit type (undefined, and in
    // practice masked to `& 31` on x86, yielding a garbage mantissa)
    if(shift > 10)
      return (dt_hdrmerge_half_t)(sign << 15);
