               ...
               const dt_iop_roi_t *const roi_in, const dt_iop_roi_t *const roi_out)
{
  dt_iop_colorcontrast_data_t *data = piece->data;
  dt_iop_colorcontrast_global_data_t *gd = self->global_data;
  if(gd->vk_kernel_colorcontrast < 0) return -1;

  struct { int w, h; float sl, sa, sb, ol, oa, ob; int unbound; } pc
    = { roi_in->width, roi_in->height,
        1.0f, data->a_steepness, data->b_steepness,
        0.0f, data->a_offset,    data->b_offset,
        data->unbound };

  dt_vk_mem_t *bufs[2] = { dev_in, dev_out };
  return dt_vulkan_enqueue_kernel_2d(0, gd->vk_kernel_colorcontrast,
...
#ifdef HAVE_VULKAN
  gd->vk_program = -1;
  gd->vk_kernel_colorcontrast = -1;
  if(dt_vulkan_running())
  {
    gd->vk_program = dt_vulkan_load_program_by_name("colorcontrast");
    if(gd->vk_program >= 0)
      gd->vk_kernel_colorcontrast = dt_vulkan_create_kernel(gd->vk_program, "colorcontrast",
                                                            2, 3 * sizeof(int) + 6 * sizeof(float),
                                                            16, 16, 1);
  }
#endif
}
...
#endif
#ifdef HAVE_VULKAN
  if(gd->vk_kernel_colorcontrast >= 0) dt_vulkan_free_kernel(gd->vk_kernel_colorcontrast);
#endif
  free(self->data);
  self->data = NULL;
}
