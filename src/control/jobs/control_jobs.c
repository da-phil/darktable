...
typedef struct dt_control_merge_hdr_t
{
  ...
  // position stay consistent (no false color on moving content).
...
// Sanitize one decoded sample on its way into the bracket. A non-finite value
// (a corrupt decode, or an alignment warp that sampled outside the frame) has to
// be stopped here: downstream it multiplies into every weighted sum it reaches,
// and since 0 * NaN is NaN even a zero weight cannot contain it, so a single bad
// sample would surface as a NaN pixel in the written DNG. Black is the safe
// substitute, because the weight envelope already gives a black sample no weight
// and the remaining frames simply carry that pixel.
static inline float _hdr_finite(const float v)
{
  return dt_isfinite(v) ? v : 0.0f;
}
...
// frames (which would demosaic into false color where content moves).
//
...
static void _hdr_compute_luma(const float *const in,
                              ...
                              const int ht)
{
  // iterate the quad grid, not the pixel grid: the maximum is shared by all four
  // members of a quad, so computing it per pixel would repeat each one four times
  DT_OMP_FOR(collapse(2))
  for(int qy = 0; qy < (ht + 1) / 2; qy++)
    for(int qx = 0; qx < (wd + 1) / 2; qx++)
    {
      const int xx = 2 * qx, yy = 2 * qy;
      const int x1 = MIN(xx + 1, wd - 1);
      const int y1 = MIN(yy + 1, ht - 1);
      float m = _hdr_finite(in[xx + (size_t)wd * yy]);
      m = MAX(m, _hdr_finite(in[x1 + (size_t)wd * yy]));
      m = MAX(m, _hdr_finite(in[xx + (size_t)wd * y1]));
      m = MAX(m, _hdr_finite(in[x1 + (size_t)wd * y1]));
      const dt_hdrmerge_half_t h = dt_hdrmerge_float_to_half(m);
      // on an odd dimension the last quad is a single row/column, and x1/y1
      // clamp back onto xx/yy, so these writes collapse onto the same cell
      out[xx + (size_t)wd * yy] = h;
      out[x1 + (size_t)wd * yy] = h;
      out[xx + (size_t)wd * y1] = h;
      out[x1 + (size_t)wd * y1] = h;
    }
}
                                        ...
                                        const gboolean export_masks)
{
  ...
  for(size_t k = 0; k < npx; k++)
    buf[k] = dt_hdrmerge_float_to_half(_hdr_finite(in_buf[k]));
