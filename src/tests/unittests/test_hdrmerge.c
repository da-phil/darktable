 ...
 *   3. a bright colored patch that clips only in the longer exposures keeps its
 *      color and brightness - a near-clipped frame is faded toward the shortest
...
// RGGB phase: (row%2,col%2) -> (0,0)=R, (1,1)=B, else G. The scene is colored so
...
// A smooth, colored, everywhere-unclipped field (longest exposure X <= 0.42).
...
// A bright colored patch that clips only in the longer exposures must keep its
// color AND its brightness: a near-clipped frame is faded toward the shortest
