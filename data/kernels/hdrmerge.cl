...
{
  // negated form so a NaN L is rejected (see hdrmerge_weight)
  if(!(L > 0.0f && L < white))
    return 0.0f;
...
//          all CFA channels; the weights are derived from this so neighboring
//          R/G/B stay consistent and do not false-color on motion. The host
               ...
               const int ref_frame)
{
  ...
  // false color); frames disagreeing with it are faded out below. Where the
  ...
  // luminance-driven, hence identical across color channels.
  num = 0.0f;
  den = 0.0f;
  for(int i = 0; i < num_frames; i++)
  {
