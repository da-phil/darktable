// Vulkan port of colorequal.cl :: bilinear2.
// Bilinear resample of a float2 buffer from (width_in, height_in) to
// (width_out, height_out). 2 bindings (in float2, out float2),
// PC: width_in, height_in, width_out, height_out = 16 B.
#include "dt_vulkan_common.h"

kernel void ce_bilinear2(global const float2 *in,
                         global float2 *out,
                         const int width_in,
                         const int height_in,
                         const int width_out,
                         const int height_out)
{
  ...
  const float2 Q_NW = in[y_prev * width_in + x_prev];
  const float2 Q_NE = in[y_prev * width_in + x_next];
  const float2 Q_SE = in[y_next * width_in + x_next];
  const float2 Q_SW = in[y_next * width_in + x_prev];
