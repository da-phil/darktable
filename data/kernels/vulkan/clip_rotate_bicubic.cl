// Vulkan port of basic.cl :: clip_rotate_bicubic.
//
// Same homography backtransform as clip_rotate_bilinear, with a 4×4
// bicubic-spline reconstruction (kwidth = 2). The OpenCL kernel uses
...
kernel void clip_rotate_bicubic(
    ...
    const float mb0, const float mb1)
{
  ...
  for(int jj = 1 - kwidth; jj <= kwidth; jj++)
    for(int ii = 1 - kwidth; ii <= kwidth; ii++)
    {
      const int i = tx + ii;
      const int j = ty + jj;
      const float wx = vk_interpolation_bicubic((float)i - px);
      const float wy = vk_interpolation_bicubic((float)j - py);
      const float w = wx * wy;
