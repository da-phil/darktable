// Vulkan port of basic.cl :: clip_rotate_lanczos2.
//
// Same homography backtransform + image-shortcut as
// clip_rotate_bicubic; only the weight function differs (4×4
// Lanczos-2 instead of bicubic spline).
...
kernel void clip_rotate_lanczos2(
    ...
    const float mb0, const float mb1)
{
  ...
  for(int jj = 1 - kwidth; jj <= kwidth; jj++)
    for(int ii = 1 - kwidth; ii <= kwidth; ii++)
    {
      const int i = tx + ii;
      const int j = ty + jj;
      const float wx = vk_interpolation_lanczos(2.0f, (float)i - px);
      const float wy = vk_interpolation_lanczos(2.0f, (float)j - py);
      const float w = wx * wy;
