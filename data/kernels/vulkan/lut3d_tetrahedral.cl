// Vulkan port of lut3d.cl :: lut3d_tetrahedral.
//
// 3D LUT lookup with tetrahedral interpolation. Splits the cube
// containing the input RGB triplet into 6 tetrahedra and selects the
// correct one by comparing rgbd.x / rgbd.y / rgbd.z; then interpolates
// across the 4 vertices of that tetrahedron. Alpha passes through.
//
// Image-shortcut port: same shape as lut3d_trilinear.
//
...
kernel void lut3d_tetrahedral(global const float4 *in,
                              global       float4 *out,
                              ...
                              const int level)
{
  ...
  if(rgbd.x > rgbd.y)
  {
    if(rgbd.y > rgbd.z)
    {
      output = (1.0f - rgbd.x) * c000 + (rgbd.x - rgbd.y) * c100
             + (rgbd.y - rgbd.z) * c110 + rgbd.z * c111;
    }
    else if(rgbd.x > rgbd.z)
    {
      output = (1.0f - rgbd.x) * c000 + (rgbd.x - rgbd.z) * c100
             + (rgbd.z - rgbd.y) * c101 + rgbd.y * c111;
    }
    else
    {
      output = (1.0f - rgbd.z) * c000 + (rgbd.z - rgbd.x) * c001
             + (rgbd.x - rgbd.y) * c101 + rgbd.y * c111;
    }
  }
  else
  {
    if(rgbd.z > rgbd.y)
    {
      output = (1.0f - rgbd.z) * c000 + (rgbd.z - rgbd.y) * c001
             + (rgbd.y - rgbd.x) * c011 + rgbd.x * c111;
    }
    else if(rgbd.z > rgbd.x)
    {
      output = (1.0f - rgbd.y) * c000 + (rgbd.y - rgbd.z) * c010
             + (rgbd.z - rgbd.x) * c011 + rgbd.x * c111;
    }
    else
    {
      output = (1.0f - rgbd.y) * c000 + (rgbd.y - rgbd.x) * c010
             + (rgbd.x - rgbd.z) * c110 + rgbd.z * c111;
    }
  }
  output.w = input.w;
  out[idx] = output;
