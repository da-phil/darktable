// Vulkan port of extended.cl :: colorbalance_lgg.
//
// Lift / gamma / gain in ProPhoto-RGB space with optional input/
// output saturation and fulcrum-around-grey contrast. Same shape
// as the LEGACY variant (push-constant-only, 2 bindings) but with
// a 16-float PC for the extra saturation_out slot.
//
// PC: 2 ints + 16 floats = 72 bytes.
...
kernel void colorbalance_lgg(global const float4 *in,
                             global       float4 *out,
                             const int   width,
                             const int   height,
                             ...
                             const float saturation_out)
{
  ...
  // Gamma 2.2 round-trip with lift/gain in between.
  RGB = (RGB <= (float4)0.0f)
            ? (float4)0.0f
            : vk_cb_pow4(RGB, (float4)(1.0f / 2.2f, 1.0f / 2.2f, 1.0f / 2.2f, 1.0f));
  RGB = ((RGB - (float4)1.0f) * lift + (float4)1.0f) * gain;
  RGB = (RGB <= (float4)0.0f)
            ? (float4)0.0f
            : vk_cb_pow4(RGB, gamma_inv * (float4)(2.2f, 2.2f, 2.2f, 1.0f));
