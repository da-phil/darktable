// Vulkan port of lut3d.cl :: lut3d_pyramid.
//
// 3D LUT lookup with pyramidal interpolation. Like tetrahedral but
// uses 3 different pyramidal decompositions of the cube based on
// which of rgbd.x/.y/.z is the largest. Alpha passes through.
//
// Image-shortcut port: same shape as the other lut3d kernels.
//
...
kernel void lut3d_pyramid(global const float4 *in,
                          global       float4 *out,
                          ...
                          const int level)
{
  ...
  {
    output = c000 + (c111 - c011) * rgbd.x + (c010 - c000) * rgbd.y
           + (c001 - c000) * rgbd.z
           + (c011 - c001 - c010 + c000) * rgbd.y * rgbd.z;
  }
  else if(rgbd.x > rgbd.y && rgbd.z > rgbd.y)
  {
    output = c000 + (c100 - c000) * rgbd.x + (c111 - c101) * rgbd.y
           + (c001 - c000) * rgbd.z
           + (c101 - c001 - c100 + c000) * rgbd.x * rgbd.z;
  }
  else
  {
    output = c000 + (c100 - c000) * rgbd.x + (c010 - c000) * rgbd.y
           + (c111 - c110) * rgbd.z
