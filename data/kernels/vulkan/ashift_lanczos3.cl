// Vulkan port of basic.cl :: ashift_lanczos3.
//
// 6×6 Lanczos-3 reconstruction; otherwise identical to ashift_lanczos2.

#include "dt_vulkan_common.h"

kernel void ashift_lanczos3(global const float4 *in,
                            global       float4 *out,
                            ...
                            const float clip_y)
{
  ...
  for(int jj = 1 - kwidth; jj <= kwidth; jj++)
    for(int ii = 1 - kwidth; ii <= kwidth; ii++)
    {
      const int i = tx + ii;
      const int j = ty + jj;
      const float wx = vk_interpolation_lanczos(3.0f, (float)i - rx);
      const float wy = vk_interpolation_lanczos(3.0f, (float)j - ry);
      const float w = wx * wy;
